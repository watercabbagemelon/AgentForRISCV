...
无需参数。自动从 `orfs-agent` MCP 读取最新结果，并扫描 `./report/` 确定当前序号。
...
扫描 `./report/` 目录，找出已有的 `report_*.md` 文件，取最大序号 +1 作为本次序号 `N`。
若目录为空，则 `N = 1`。

```
list_files_container 或直接读取宿主机 ./report/ 目录
```
...
将收集到的数据填入报告模板，写入 `./report/report_<N>.md`：

```
write_file("report/report_<N>.md", <报告内容>)
```
...
```
write_file("report/log_<N>.md", <优化日志内容>)
```
...
write_file("sky130hd/<design_nickname>/constraint.sdc", <新内容>)
```

**面积/拥塞优化：**
```
read_file("sky130hd/<design_nickname>/config.mk")
write_file("sky130hd/<design_nickname>/config.mk", <新内容>)
```
...
> 本报告对应优化日志：`report/log_<N>.md`（如有优化）
```
...
**对应报告：** report/report_<N>.md（本轮优化后生成）
**上一轮报告：** report/report_<N-1>.md（优化前基准）
