...
module UARTRx_1(
  ...
  always @(posedge clock) begin
    if (reset) begin
      ...
      valid <= 1'h0;	// UARTRx.scala:109:22, :172:9
      state <= 1'h0;	// UARTRx.scala:115:22, :172:9
    end
    else begin
      ...
      else begin	// UARTRx.scala:115:22
        if (io_in) begin
    ...
    if (_GEN_1)	// UARTRx.scala:115:22, :123:21, :125:29, :126:17
      prescaler <=
        (restore ? 13'h36 : prescaler) - {12'h0, ~(restore & sample_count < 4'h4)};	// UARTRx.scala:64:22, :74:25, :89:25, :90:30, :99:23, :100:25, :101:{37,42,51}
    ...
    if (~state | ~_GEN_3 | data_last) begin	// UARTRx.scala:72:31, :107:20, :110:9, :115:22, :117:18, :139:20, :145:27, :160:30
      ...
      shifter <=
        {sample[0] & sample[1] | sample[0] & sample[2] | sample[1] & sample[2],
         shifter[7:1]};	// Cat.scala:33:92, Misc.scala:166:48, :167:22, UARTRx.scala:103:19, :105:31, :107:20, :164:45
  end // always @(posedge)
  `ifndef SYNTHESIS
