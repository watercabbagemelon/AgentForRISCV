...
const VSRC_DIR       = path.join(PROJECT_ROOT, "vsrc");
const RTL_OUTPUT_DIR = path.join(VSRC_DIR, "rtl_output");
  ...
  async ({ config, extra_make_args }) => {
    try {
      ...
      const makeCmd = [
        ...
        `mkdir -p /workspace/vsrc/rtl_output`,
        `cp -r generated-src/chipyard.TestHarness.${config} /workspace/vsrc/rtl_output/`,
      ].join(" && ");
      ...
      // 输出：vsrc/rtl_output/chipyard.TestHarness.<config>/verilog_synth/
      const firFile = `/workspace/vsrc/rtl_output/chipyard.TestHarness.${config}/chipyard.TestHarness.${config}.fir`;
      const synthOutDir = `/workspace/vsrc/rtl_output/chipyard.TestHarness.${config}/verilog_synth`;
      ...
      return {
        content: [{
          ...
          text: [
            ...
            ``,
            `原始 SV 输出: vsrc/rtl_output/chipyard.TestHarness.${config}/gen-collateral/`,
            `可综合 Verilog: vsrc/rtl_output/chipyard.TestHarness.${config}/verilog_synth/  (${synthCount} 个文件)`,
...
server.tool(
  ...
  async ({ config }) => {
    try {
      // mems.conf 路径：vsrc/rtl_output/<config>/<config>.mems.conf
  ...
  async ({ config, subdir }) => {
    try {
      ...
      return {
        content: [{
          type: "text",
          text: `目录 [vsrc/rtl_output/chipyard.TestHarness.${config}/${subdir}] 内容:\n${files.join("\n")}`,
        }],
      };
    } catch (error) {
