...
// 设计配置路径（容器内 ORFS flow 目录下的相对路径）
const DESIGN_CONFIG = "designs/sky130hd/tinyRocket/config.mk";
const DESIGN_PLATFORM = "sky130hd";
const DESIGN_NICKNAME = "tinyRocket";
// 宿主机项目根目录（用于 read_file/write_file 路径限制）
const PROJECT_ROOT = path.resolve(new URL("../../", import.meta.url).pathname);

// ORFS 容器启动命令参考（需在外部或 skill 中执行）:
...
//
// 设计文件（config.mk, constraint.sdc, sram_macros.v）需从宿主机
// /workspace/sky130hd/<nickname>/ 部署到容器内 designs/sky130hd/<nickname>/
...
server.tool(
  ...
  async ({ stage }) => {
    try {
      ...
      // WSL2 环境下 CTS 后 detailed_placement 会崩溃，需加 SKIP_CTS_REPAIR_TIMING=1
      const skipFlag = "SKIP_CTS_REPAIR_TIMING=1";
      const cmd = `docker exec ${CONTAINER_NAME} bash -c "cd ${ORFS_FLOW_DIR} && make DESIGN_CONFIG=${DESIGN_CONFIG} ${skipFlag} ${stage}"`;
      const output = execSync(cmd, { encoding: "utf-8", timeout: 3600000 }); // 1h timeout
      return {
...
server.tool(
  ...
  async () => {
    try {
      const cmd = `docker exec ${CONTAINER_NAME} cat ${ORFS_FLOW_DIR}/results/${DESIGN_PLATFORM}/${DESIGN_NICKNAME}/base/metadata.json`;
  ...
  async ({ path: filePath }) => {
    try {
      ...
      if (!fullPath.startsWith(PROJECT_ROOT)) {
        return { isError: true, content: [{ type: "text", text: `拒绝访问项目目录之外的路径: ${filePath}` }] };
      }
...
server.tool(
  ...
  async ({ path: filePath, content }) => {
    try {
      const fullPath = path.resolve(PROJECT_ROOT, filePath);
      if (!fullPath.startsWith(PROJECT_ROOT)) {
        return { isError: true, content: [{ type: "text", text: `拒绝写入项目目录之外的路径: ${filePath}` }] };
      }
